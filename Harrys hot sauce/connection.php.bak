<html>
	<title> Heather's Connection Page </title>
	<body><center> Heather's Connection Page </center>
		<p>
			<?PHP
			...
			{
				die('Could not connect: ' . mysqli_connect_errno($link). " -
			".mysqli_connect_error($link)."<BR>");
			}
			else 
			 	{
				...
				else 
				{
					//retrieve and print first row of data only
					 ...
					 while($array = mysqli_fetch_array($result,MYSQLI_BOTH))
					  {
					  	print("<FONT COLOR='Red'>$array[0]  $array[1]
					 $array[2]</FONT><BR>");					  	
					 		 }
						}
